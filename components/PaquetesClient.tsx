...
function PaquetesClientContent({ paquetes, categorias }: PaquetesClientProps) {
  ...
  const destinationOptions = useMemo(() => {
    return categoriasDisponibles
      .map((categoria) => ({
        id: categoria.id,
        slug: normalizeFilterValue(categoria.slug || categoria.nombre || ''),
        nombre: String(categoria.nombre || '').trim(),
      }))
      .filter((categoria) => categoria.slug && categoria.nombre);
  }, [categoriasDisponibles]);

  const destinationOptionByValue = useMemo(() => {
    ...
    for (const destination of destinationOptions) {
      ...
      map.set(normalizeFilterValue(destination.nombre), destination);
    }
    return map;
  }, [destinationOptions]);

  const destinationIdBySlug = useMemo(() => {
    return new Map(destinationOptions.map((destination) => [destination.slug, destination.id]));
  }, [destinationOptions]);
  ...
  useEffect(() => {
    ...
    const urlDestino = normalizeMulti([
      ...readMulti(urlParams, 'destino'),
      ...readMulti(urlParams, 'categoria'),
    ])
      .map((value) => destinationOptionByValue.get(normalizeFilterValue(value))?.slug || normalizeFilterValue(value))
      .filter(Boolean);
  ...
  }, [destinationOptionByValue, searchParams]);
  ...
  const selectedDestinationIds = useMemo(() => {
    return selectedDestinos
      .map((destinationSlug) => destinationIdBySlug.get(normalizeFilterValue(destinationSlug)) || '')
      .filter(Boolean);
  }, [destinationIdBySlug, selectedDestinos]);
  ...
  const paquetesFiltrados = useMemo(() => {
    let list = paquetes.filter((paquete) => {
      ...
      if (selectedDestinationIds.length > 0) {
        const matchesDestination = selectedDestinationIds.some((categoriaId) => packageHasCategory(paquete, categoriaId));
    ...
    if (sortBy === 'nombre') {
      ...
      list = [...list].sort((a, b) => {
        const da = normalizeFilterValue(a.destino || a.eventoLugar || '');
        const db = normalizeFilterValue(b.destino || b.eventoLugar || '');
        return da.localeCompare(db);
      });
    }
    return list;
  }, [paquetes, searchTerm, selectedDestinationIds, selectedTipos, selectedTransportes, selectedTags, selectedIncluye, sortBy, slug, mes]);
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {destinos.map((destino) => (
                        <label key={destino.id} className="flex items-center gap-2.5 cursor-pointer group">
                          <Checkbox
                            checked={selectedDestinos.includes(destino.slug)}
                            onCheckedChange={() => toggleDestino(destino.slug)}
                            className="border-gray-300 data-[state=checked]:bg-black data-[state=checked]:border-black"
                          />
                          <span className="text-sm text-gray-700 group-hover:text-black">{destino.nombre}</span>
                        </label>
                      ))}
                    </div>
                  </div>
