...
import { fetchActiveCategorias } from '@/lib/packages/admin-queries';
...
import { Plus, Loader2, Search, X, User, Users, Sparkles, Globe2, GraduationCap, CheckCircle2, XCircle, Star, Music } from 'lucide-react';
...
export default function PaquetesPage() {
  ...
  const fetchPaquetes = async () => {
    try {
      const [snapshot, categorias] = await Promise.all([
        getDocs(query(collection(db, 'paquetes'), orderBy('orden', 'asc'))),
        fetchActiveCategorias(),
      ]);
      const data = snapshot.docs.map((doc) =>
        syncPackageCategoryData({ id: doc.id, ...doc.data() } as Paquete, categorias)
      );
      setPaquetes(data);
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (paqueteDoc.exists()) {
        ...
        if ((paqueteData as any).imagenPortadaMobileKey) keysToDelete.add((paqueteData as any).imagenPortadaMobileKey);
        if ((paqueteData as any).imagenPortadaDesktopKey) keysToDelete.add((paqueteData as any).imagenPortadaDesktopKey);
        ...
        const portadaMobileFallback = getBlobKeyFromUrl((paqueteData as any).imagenPortadaMobile || '');
        if (portadaMobileFallback) keysToDelete.add(portadaMobileFallback);
        const portadaDesktopFallback = getBlobKeyFromUrl((paqueteData as any).imagenPortadaDesktop || '');
        if (portadaDesktopFallback) keysToDelete.add(portadaDesktopFallback);
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      await revalidateFrontPaths(['/excursiones', `/excursion/${result.paquete.slug}`]);
        ...
        <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
          <AlertDialogContent className="z-[101]">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Esta accion no se puede deshacer. Se eliminara la excursion permanentemente.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
